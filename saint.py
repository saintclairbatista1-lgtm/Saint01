...
def init_db():
    ...
    cursor.execute('SELECT COUNT(*) FROM ledger')
    if cursor.fetchone()[0] == 0:
        genesis_payload = "Genesis Block (S Message Multicurrency Wallet + SQLite)"
        genesis_hash = "0" * 128
        cursor.execute('''
            INSERT INTO ledger (block_index, timestamp, payload, previous_hash, block_hash)
            VALUES (?, ?, ?, ?, ?)
...
class Custom500ByteProcessor:
    ...
    def execute_aes_encrypt(self, plaintext_bytes: bytes, nonce_bytes: bytes) -> dict:
        ...
        ciphertext = aesgcm.encrypt(nonce_bytes, plaintext_bytes, None)
        return {
            "processor_status": "OK (500-Byte ALU Active + Wallet Ledger)",
            "ciphertext_hex": ciphertext.hex(),
            "nonce_hex": nonce_bytes.hex(),
            "register_capacity": "500 Bytes (4000 bits)"
        }
...
# ==========================================
app = FastAPI(
    title="S Message - Autonomous 500-Byte Secure Ledger & Multicurrency Wallet",
    version="2.8.0",
    description="API blindada com microprocessador de 500 bytes, biometria e carteira multimoeda (SDC, SDT, USD, EUR, BRL) via SQLite"
)

MILITARY_GRADE_SECRET = b"s-message-secure-master-key-2026"
request_history = defaultdict(list)
RATE_LIMIT_WINDOW = 60
SUPPORTED_ASSETS = ["SDC", "SDT", "USD_DIGITAL", "EUR_DIGITAL", "BRL_DIGITAL"]
...
async def military_grade_shield(request: Request, call_next):
    ...
    if len(request_history[client_ip]) >= 15:
        return JSONResponse(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            content={"detail": "Blindagem Ativa: Limite de requisições excedido."}
        )
...
# ==========================================
# 5. ROTAS DA API (STATUS E WALLET)
# ==========================================
...
@app.post("/api/v1/secure-transfer-biometric", summary="Transferência Segura Multimoeda com Biometria e Ledger")
async def secure_transfer_biometric(
    asset_code: str = Form(..., description="Ex: SDC, USD_DIGITAL, BRL_DIGITAL"),
    amount: float = Form(..., description="Valor da transação"),
    wallet_from: str = Form(..., description="Carteira de origem"),
    wallet_to: str = Form(..., description="Carteira de destino"),
    digital_signature: str = Form(..., description="Assinatura HMAC-SHA256"),
    ...
    # 3. Operação de Saldos no SQLite (Transacional)
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    ...
    # Credita no destino (garante criação se não existir)
    ...
    # 4. Criptografia via Microprocessador de 500 Bytes
    nonce = os.urandom(12)
    encrypted_data = cpu_500bytes.execute_aes_encrypt(
        json.dumps(payload, sort_keys=True).encode('utf-8'), 
        nonce
    ...
    # 5. Leitura do último bloco do Ledger para encadeamento
    cursor.execute('SELECT block_index, block_hash FROM ledger ORDER BY block_index DESC LIMIT 1')
    last_block = cursor.fetchone()
    ...
    # 6. Cálculo do Hash do novo bloco
    block_string = json.dumps({
        "index": new_index,
        "timestamp": new_timestamp,
        "payload": encrypted_data,
    ...
    # 7. Gravação permanente no Ledger SQLite
    cursor.execute('''
        INSERT INTO ledger (block_index, timestamp, payload, previous_hash, block_hash)
        VALUES (?, ?, ?, ?, ?)
    ''', (new_index, new_timestamp, json.dumps(encrypted_data), previous_hash, new_block_hash))
    ...
    return {
        "status": "200 OK",
        "message": f"Transferência de {amount} {asset_code} processada, validada por biometria e gravada no Ledger!",
        "block_index": new_index,
        "processor_telemetry": encrypted_data["processor_status"],
        "encrypted_envelope": {
            "ciphertext_hex": encrypted_data["ciphertext_hex"],
